...
void led_set_ALLRGB(uint32_t *color) {
#if (NUM_BPP == 4) // SK6812
	for (uint8_t index = 0; index < NUM_PIXELS; index++) {
		uint8_t rled = (color[index] >> 24) & 0xff;
		uint8_t gled = (color[index] >> 16) & 0xff;
		uint8_t bled = (color[index] >> 8)  & 0xff;
		rgb_arr[4 * index] = scale8(gled, 0xB0); // g;
		rgb_arr[4 * index + 1] = rled;
		rgb_arr[4 * index + 2] = scale8(bled, 0xF0); // b;
		rgb_arr[4 * index + 3] = 0;
	}
#else // WS2812B
	for (uint8_t index = 0; index < NUM_PIXELS; index++) {
		...
		uint8_t gled = (color[index] >> 16) & 0xff;
		uint8_t bled = (color[index] >> 8)  & 0xff;
		rgb_arr[3 * index] = scale8(gled, 0xB0); // g;
		rgb_arr[3 * index + 1] = rled;
		rgb_arr[3 * index + 2] = scale8(bled, 0xF0); // b;
	}
#endif // End SK6812 WS2812B case differentiation
}
...
void led_render() {
  if(wr_buf_p != 0 || hdma_tim3_ch1_trig.State != HAL_DMA_STATE_READY) {
  ...
  HAL_TIM_PWM_Start_DMA(&htim3, TIM_CHANNEL_1, (uint32_t *)wr_buf, WR_BUF_LEN);
